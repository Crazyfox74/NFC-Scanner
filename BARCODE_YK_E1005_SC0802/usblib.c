...
volatile USBLIB_TypeDef *USB_DEV = (USBLIB_TypeDef *)USBLIB_BASE;
volatile USBLIB_EPBuf EPBufTable[EPCOUNT] __attribute__((at(USB_PBUFFER)));
volatile uint32_t     USBEP[EPCOUNT] __attribute__((at(USBLIB_BASE)));
...
void USBLIB_Init(void)
{
    ...
    USB_DEV->CNTR   = USB_CNTR_FRES; /* Force USB Reset */
    USB_DEV->BTABLE = 0;
    USB_DEV->DADDR  = 0;
    USB_DEV->ISTR   = 0;
    USB_DEV->CNTR   = USB_CNTR_RESETM;
...
void USBLIB_Reset(void)
{
    ...
    for (uint8_t i = 0; i < EPCOUNT; i++) {
        ...
        USB_DEV->EPR[i] = (EpData[i].Number | EpData[i].Type | RX_VALID | TX_NAK);
    }

    for (uint8_t i = EPCOUNT; i < 8; i++) {
        USB_DEV->EPR[i] = i | RX_NAK | TX_NAK;
    }
    USB_DEV->CNTR   = USB_CNTR_CTRM | USB_CNTR_RESETM | USB_CNTR_SUSPM;
    USB_DEV->ISTR   = 0x00;
    USB_DEV->BTABLE = 0x00;
    USB_DEV->DADDR  = USB_DADDR_EF;
}

void USBLIB_setStatTx(uint8_t EPn, uint16_t Stat)
{
    register uint16_t val = USB_DEV->EPR[EPn];
    USB_DEV->EPR[EPn]         = (val ^ (Stat & EP_STAT_TX)) & (EP_MASK | EP_STAT_TX);
}

void USBLIB_setStatRx(uint8_t EPn, uint16_t Stat)
{
    register uint16_t val = USB_DEV->EPR[EPn];
    USB_DEV->EPR[EPn]         = (val ^ (Stat & EP_STAT_RX)) & (EP_MASK | EP_STAT_RX);
}
...
void USBLIB_EPHandler(uint16_t Status)
{
    ...
    if (EP & EP_CTR_TX) { //something transmitted
        if (DeviceAddress) {
            USB_DEV->DADDR    = DeviceAddress | 0x80;
            DeviceAddress = 0;
        }
...
{
    if (USB_DEV->ISTR & USB_ISTR_RESET) { // Reset
        USB_DEV->ISTR &= ~USB_ISTR_RESET;
        USBLIB_Reset();
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_CTR) { //Handle data on EP
        USBLIB_EPHandler((uint16_t)USB_DEV->ISTR);
        USB_DEV->ISTR &= ~USB_ISTR_CTR;
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_PMAOVR) {
        USB_DEV->ISTR &= ~USB_ISTR_PMAOVR;
        // Handle PMAOVR status
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_SUSP) {
        USB_DEV->ISTR &= ~USB_ISTR_SUSP;
        if (USB_DEV->DADDR & 0x7f) {
            USB_DEV->DADDR = 0;
            USB_DEV->CNTR &= ~ 0x800;
        }
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_ERR) {
        USB_DEV->ISTR &= ~USB_ISTR_ERR;
        // Handle Error
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_WKUP) {
        USB_DEV->ISTR &= ~USB_ISTR_WKUP;
        // Handle Wakeup
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_SOF) {
        USB_DEV->ISTR &= ~USB_ISTR_SOF;
        // Handle SOF
        return;
    }
    if (USB_DEV->ISTR & USB_ISTR_ESOF) {
        USB_DEV->ISTR &= ~USB_ISTR_ESOF;
        // Handle ESOF
        return;
    }
    USB_DEV->ISTR = 0;
